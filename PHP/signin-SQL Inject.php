<?php
$con = mysqli_connect("localhost", "root", "", "music");$cart_count = 0;$errorMsg = "";
if (isset($_POST['submit'])) {
    if (array_key_exists('first_name', $_POST) && array_key_exists('last_name', $_POST) &&
        ...
        array_key_exists('postcode', $_POST) && array_key_exists('country', $_POST)) {        $firstname = mysqli_real_escape_string($con, $_POST['first_name']);
        ...
        $country = mysqli_real_escape_string($con, $_POST['country']);        // Database connection
        if($con->connect_error){
            echo "$con->connect_error";
            die("Connection Failed : ". $con->connect_error);
        } else {
            $sql = "SELECT * FROM customer WHERE email = '$email'";
            $execute = mysqli_query($con, $sql);            if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
